...
import com.example.myapp.LoginSQLiteActivity;
...
public class LaunchFragment extends Fragment {
    ...
    private int mCount = 2; // 引导页的数量

    // 获取该碎片的一个实例
    public static com.example.myapp.fragment.LaunchFragment newInstance(int position, int image_id) {
        com.example.myapp.fragment.LaunchFragment fragment = new com.example.myapp.fragment.LaunchFragment(); // 创建该碎片的一个实例
...
//            });
        ...
        // 当 btn_start 被点击时跳转到商城登陆页面
        btn_start.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                Intent intent = new Intent();
                intent.setClass(getActivity(), LoginSQLiteActivity.class);
