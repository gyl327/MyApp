...
import com.example.myapp.bean.UserInfo;
...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    private static com.example.myapp.database.UserDBHelper mHelper = null; // 数据库帮助器的实例
    ...
    // 利用单例模式获取数据库帮助器的唯一实例
    public static com.example.myapp.database.UserDBHelper getInstance(Context context, int version) {
        if (version > 0 && mHelper == null) {
            mHelper = new com.example.myapp.database.UserDBHelper(context, version);
        } else if (mHelper == null) {
            mHelper = new com.example.myapp.database.UserDBHelper(context);
        }
        return mHelper;
    }
