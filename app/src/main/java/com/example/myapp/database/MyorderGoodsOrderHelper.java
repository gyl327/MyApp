...
public class MyorderGoodsOrderHelper extends MyorderDbHelper {
    private static String table_name = "goods_myorder";

    public MyorderGoodsOrderHelper(Context context) {
        super(context, db_name, null, 1);
        mTableName = table_name;
        mSelectSQL = String.format("select rowid,_id,goods_name,price,num,sum,create_time from %s where "
                , mTableName);
    }

    private static com.example.myapp.database.MyorderGoodsOrderHelper mHelper = null; // 数据库帮助器的实例
    // 利用单例模式获取数据库帮助器的唯一实例
    public static com.example.myapp.database.MyorderGoodsOrderHelper getInstance(Context context) {
        if (mHelper == null) {
            mHelper = new com.example.myapp.database.MyorderGoodsOrderHelper(context);
        }
        return mHelper;
    }
...
//            cv.put("evaluate_status", info.evaluate_status);
    ...
    public List<MyorderGoodOrder> query(String condition) {
        ...
        while (cursor.moveToNext()) {
            ...
            info.num = cursor.getDouble(3);
            info.sum = cursor.getDouble(3);
