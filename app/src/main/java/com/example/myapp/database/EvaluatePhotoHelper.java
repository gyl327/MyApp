...
public class EvaluatePhotoHelper extends DbHelper {
    ...
    private static com.example.myapp.database.EvaluatePhotoHelper mHelper = null; // 数据库帮助器的实例
    // 利用单例模式获取数据库帮助器的唯一实例
    public static com.example.myapp.database.EvaluatePhotoHelper getInstance(Context context) {
        if (mHelper == null) {
            mHelper = new com.example.myapp.database.EvaluatePhotoHelper(context);
        }
        return mHelper;
    }
