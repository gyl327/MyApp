...
import com.example.myapp.bean.EvaluatePhoto;
import com.example.myapp.bean.GoodsOrder;
import com.example.myapp.database.EvaluateInfoHelper;
import com.example.myapp.database.EvaluatePhotoHelper;
...
public class EvaluateGoodsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void openSelectDialog(View v) {
        ...
        values.put(MediaStore.Video.Media.DISPLAY_NAME, "photo_"+ DateUtil2.getNowDateTime());
    ...
    private void addBitmapList(Bitmap bitmap, int iv_id, int pos, ImageView iv_next) {
        if (iv_current.getId() == iv_id) {
            ...
            String filePath = String.format("%s/%s.jpg",
                    getExternalFilesDir(Environment.DIRECTORY_PICTURES), "photo_"+ DateUtil2.getNowDateTime());
    ...
    private void saveEvaluateRecord() {
        ...
        info.create_time = DateUtil2.getNowDateTime();
